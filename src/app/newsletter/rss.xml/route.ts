...
export async function GET() {
  ...
  const items = await Promise.all(
    issues.map(async (issue) => {
      const full = await getNewsletter(issue.slug, "en");
      const html = absolutizeRelativeLinks(full.contentHtml, siteUrl);
      const link = `${siteUrl}/newsletter/${issue.slug}`;
      return `
      <item>
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/">
  <channel>
    <title>AI Signals Weekly</title>
    <link>${siteUrl}/newsletter</link>
    <description>Weekly digest: model launches, agent patterns, and practical tutorials.</description>
    ${items.join("\n")}
  </channel>
</rss>`;
